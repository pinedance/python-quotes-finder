...
def build_matrix(a, b, match_score=3, gap_cost=2, debug=False, log_disable=False ):

    len_a = len(a)
    len_b = len(b)
    if not log_disable: print( "* Complexity: {:,} ({:,} × {:,})".format( len_a * len_b, len_a, len_b ) )
    H, P = {}, {}
    ...
    for i in tqdm( range( 1, len_a+1 ), disable=log_disable ):
...
def smith_waterman(a, b, match_score=3, gap_cost=2, min_len=8, overlap=False, debug=False, log_disable=False ):
    """
    a : source
    b : target
    """
    ...
    if not log_disable: print( "* Build Matrix ... ")
    _q = time()
    H, P = build_matrix(a, b, match_score, gap_cost, debug=debug, log_disable=log_disable )
    H_lst = H.items()
    H_sorted = sorted( H_lst, key=lambda x: ( x[1], x[0][1] ), reverse=True )
    if not log_disable: print( "  ... {:0.3f}".format( time()-_q ) )
    if debug: print( H_sorted )

    # get all sets
    if not log_disable: print( "* Traceback ... ")
    _q = time()
    quotes_all = []
    # traceback history를 기억해 두었다가 경로가 중첩되는 경우는 버림
    trace_history = {}
    for (i, j), value in tqdm( H_sorted, disable=log_disable ):
    ...
    if not log_disable: print( "* Complete! ")
    return sorted( quotes_all, key=lambda x: (x[1][0], -x[1][1]) )

def remove_overlap(trg, indices, log_disable=True):
    # remove overlap ranges
    if not log_disable: print( "* Remove Overlap ... ")
    _q = time()

    occupation = [0] * (len( trg ) + 1)
    quotes = []
    for elem in tqdm( indices, disable=log_disable ):
        (_, _), (b_j, e_j) = elem
