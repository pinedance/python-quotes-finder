...
def save_html(a, b, indices, filename ):
    ...
    for ( i_b, i_e ), ( j_b, j_e)  in indices:
        ...
        handler.write( "\t<p class='rtg'>TRG：{}<span class='match'>{}</span>{}</p>"\
            .format( b[j_ini:j_b], b[j_b:j_e], b[j_e:j_e+border_size] ) )
        handler.write( "\t<p class='ref'>SRC：{}<span class='match'>{}</span>{}</p>"\
            .format( a[i_ini:i_b], a[i_b:i_e], a[i_e:i_e+border_size] ) )
        handler.write( "\n</div>" )
        handler.write( "<br><br>")
        i += 1
